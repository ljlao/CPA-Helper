...
const accountTotalCount = computed(() => accountSummary.total)
const enabledAccountCount = computed(() => accountSummary.enabled)
const disabledAccountCount = computed(() => accountSummary.disabled)
const unauthorizedErrorAccountCount = computed(
  () => accountSummary.unauthorized,
)
const quotaExhaustedAccountCount = computed(
  () => accountSummary.quotaExhausted,
)
...
function applyAccountSummary(accountResponse: CodexKeeperAccountsResponse) {
  accountSummary.total = accountResponse.total
  accountSummary.enabled = accountResponse.enabled_count
  accountSummary.disabled = accountResponse.disabled_count
  accountSummary.unauthorized = accountResponse.unauthorized_count
  accountSummary.quotaExhausted = accountResponse.quota_exhausted_count
}

function keeperInspectionAccountsQuery() {
...
async function loadAll() {
  ...
  try {
    const [settings, nextStatus, accountResponse] = await Promise.all([
      getCodexKeeperSettings(),
      getCodexKeeperStatus(),
      listCodexKeeperAccounts(keeperInspectionAccountsQuery()),
    ])
    applySettings(settings)
    status.value = nextStatus
    applyAccountSummary(accountResponse)
  } catch (error) {
