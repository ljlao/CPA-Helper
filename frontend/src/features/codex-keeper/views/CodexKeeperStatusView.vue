...
const displaySizeHelpText = computed(() =>
  isTableView.value
    ? t(
        ...
        `Query ${accountDisplaySize.value} accounts per page from the backend.`,
      )
    : t(
        `卡片视图每页从后台查询 ${accountDisplaySize.value} 个账号。`,
        `Card view queries ${accountDisplaySize.value} accounts per page from the backend.`,
      ),
)
...
function accountDisplayText(
  ...
  return t(
    `第 ${clampPage(accountPage.value, accountTotalPageCount.value)} / ${accountTotalPageCount.value} 页，显示 ${visibleCount} / ${totalCount} 个账号`,
    `Page ${clampPage(accountPage.value, accountTotalPageCount.value)} / ${accountTotalPageCount.value}, showing ${visibleCount} / ${totalCount} accounts`,
  )
}
...
const bulkDeleteWarningText = computed(() =>
  bulkDeleteDialog.source === 'disabled401'
    ? t(
        ...
        `This will delete ${selectedDisabledCount.value} HTTP 401 disabled accounts on the current page and remove their auth files from CPA. This cannot be undone.`,
      )
    : t(
...
const usageStatsPeriodColumns = computed<DataTableColumns<CodexKeeperUsageStatsPeriod>>(() => [
  {
    title: t('周期', 'Period'),
    key: 'label',
    width: 168,
            ...
            <p class="toolbar-subtitle">
              {{
                t(
                  ...
                  `Normal ${formatInteger(accountListMeta.filteredEnabledCount)} / ${formatInteger(enabledAccountCount)} accounts`,
                )
              }}
              <template v-if="hasDisabledAccounts">
                {{
                  t(
                    ...
                    `, disabled ${formatInteger(accountListMeta.filteredDisabledCount)} / ${formatInteger(disabledAccountCount)} accounts`,
                  )
                }}
              </template>
            </p>
              ...
              >
                ...
                {{
                  t(
                    ...
                    `Bulk Delete Current Page 401 (${filteredUnauthorizedDisabledAccounts.length})`,
                  )
                }}
              </NButton>
            </div>
